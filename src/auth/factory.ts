...
const connectionCache: Map<string, IConnectionAdapter> = new Map();

export async function createConnection(
  orgAlias: string,
  options?: ConnectionOptions
): Promise<IConnectionAdapter> {
  ...
  if (!orgConfig) {
    ...
    throw new Error(
      `Org '${orgAlias}' not found in configuration. ` +
...
export function getConnection(orgAlias: string): IConnectionAdapter {
  const cached = connectionCache.get(orgAlias);
  
  if (!cached) {
    throw new Error(
      `No active connection to org '${orgAlias}'. ` +
...
export async function closeConnection(orgAlias: string): Promise<void> {
  const cached = connectionCache.get(orgAlias);
...
export function hasConnection(orgAlias: string): boolean {
