...
const confirmedOrgs: Set<string> = new Set();
...
export function isProductionConfirmed(orgAlias: string): boolean {
  return confirmedOrgs.has(orgAlias);
}

export function confirmProductionAccess(orgAlias: string, token: string): boolean {
  const isProduction = isProductionOrg(orgAlias);
