export function enrichField(field) {
    const base = {
        apiName: field.name || '',
        label: field.label || '',
        type: field.type || '',
    ...
    if (field.calculatedFormula) {
        ...
        base.formulaTreatNullAsBlank = field.formulaTreatNullAsBlank === 'Blank';
    ...
    if (field.type === 'picklist' || field.type === 'multipicklist') {
        base.picklistValues = (field.picklistValues || []).map((pv) => ({
            value: pv.value || '',
            label: pv.label || '',
...
export function enrichObjectDescribe(describe) {
    return {
        ...
        recordTypeInfo: describe.recordTypeInfos?.map((rt) => ({
